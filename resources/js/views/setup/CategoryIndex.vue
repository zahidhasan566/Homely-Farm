<template>
  <div class="container-fluid">
    <breadcrumb :options="['Users']">
      <button class="btn btn-primary" @click="addUserModal()">Add Category</button>
    </breadcrumb>
...
<!--        <a href="javascript:" @click="changePassword(row.item.UserId)"> <i class="ti-lock"></i></a>-->
    ...
    <add-edit-category @changeStatus="changeStatus" v-if="loading"/>
  </div>
</template>
<script >
...
export default {
  ...
  methods: {
    changeStatus() {
      ...
      this.loading = true;
      setTimeout(() => {
        bus.$emit('add-edit-category', row);
      })
    },
    exportData() {
