<template>
  <div class="container-fluid">
    <breadcrumb :options="['Item']">
      <button class="btn btn-primary" @click="addUserModal()">Add Item</button>
    </breadcrumb>
...
<!--        <a href="javascript:" @click="changePassword(row.item.UserId)"> <i class="ti-lock"></i></a>-->
    ...
    <add-edit-item @changeStatus="changeStatus" v-if="loading"/>
  </div>
</template>
<script >
...
export default {
  ...
  methods: {
    changeStatus() {
      ...
      this.loading = true;
      setTimeout(() => {
        bus.$emit('add-edit-item', row);
      })
    },
    exportData() {
      bus.$emit('export-data','item-list-'+moment().format('YYYY-MM-DD'))
    }
  }
}
</script>
